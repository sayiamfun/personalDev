...
public class PersonalNoMessageController {
    ...
    public R addKeyWord(
            ...
            @RequestBody PersonalNoMessage message
    ){
        try {
    ...
    public R deleteById(
        ...
        try {
            String getSql = DaoGetSql.getSql("DELETE FROM "+DBMessageContent+" where message_id = ?",messageId);
            Sql sql = new Sql(getSql);
            messageContentService.deleteBySql(sql);
            getSql = DaoGetSql.deleteById(DBMessage,messageId);
            sql.setSql(getSql);
